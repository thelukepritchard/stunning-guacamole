...
/**
 * Lambda entry-point for the account domain. Routes the incoming API Gateway
...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  switch (routeKey) {
    case 'POST /feedback':     return submitFeedback(event);
    case 'DELETE /account':    return deleteAccount(event);
    default:
      return {
        statusCode: 404,
        headers: {
