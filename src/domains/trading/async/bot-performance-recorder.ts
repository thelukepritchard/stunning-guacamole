...
import type { BotRecord, TradeRecord, BotPerformanceRecord } from '../types';
...
function calculatePnl(
  trades: TradeRecord[],
  currentPrice: number,
): Pick<BotPerformanceRecord, 'totalBuys' | 'totalSells' | 'totalBuyValue' | 'totalSellValue' | 'realisedPnl' | 'unrealisedPnl' | 'netPnl' | 'netPosition'> {
  let totalBuys = 0;
  let totalSells = 0;
  let totalBuyValue = 0;
