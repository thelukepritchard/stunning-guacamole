...
- **AuthStack** (`lib/auth.ts`) — Cognito User Pool (self-sign-up disabled) and User Pool Client.
...
## Standards
- All Lambda functions are developed in NodeJS 24
...
1. Create the domain handler directory at `src/domains/<name>/` with:
   - `utils.ts` — shared `jsonResponse` helper and route handler types.
   - `routes/<route>.ts` — one file per route, each exporting a single handler function.
   - `index.ts` — Lambda entry-point that imports routes and dispatches via a `switch` on `${event.httpMethod} ${event.resource}`.
2. Create `lib/domain-<name>.ts` using `NodejsFunction` pointing to the handler entry file.
3. Accept `api` and `authorizer` via props; add `ANY` methods on the root resource and any sub-resources (e.g. `{id}`).
4. Wire the new stack in `bin/infrastructure.ts`, passing the REST API and authorizer.
